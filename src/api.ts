...
export type AddressClaimChallenge = { id: string; token: string; wallet: string; launchId: string; kind: "creator" | "cumulative" | "legacy"; epochId: string | null; depositAddress: string; amountLamports: number; expiresAt: number };
...
export const api = {
  ...
  claimHistory: (wallet: string) => uncachedRequest<{lifetimeUsdCents?:string|null;lifetime:Array<{stock_mint:string;symbol:string;decimals:number;amount_raw:string;usd_cents?:string;usd_complete?:boolean}>;hasMore:boolean;claims:Array<{launch_id:string;name:string;symbol:string;reward_symbol:string;stock_decimals:number;amount_raw:string;usd_cents?:string;usd_complete?:boolean;signature:string;claimed_at:number;kind:string}>}>(`/api/wallets/${encodeURIComponent(wallet)}/claim-history`),
  ...
  submitLaunchBatch: (id: string, transactions: SignedTransactionEnvelope[], signal?: AbortSignal) => request<LaunchRelayStatus>(`/api/launches/${encodeURIComponent(id)}/submit-batch`, { ...json({ sequential:true, transactions: transactions.map(({ step, signedTransactionBase64 }) => ({ step, signedTransactionBase64 })) }), signal }),
  ...
  adminRipple:(token:string,search:string,offset=0,signal?:AbortSignal)=>request<import('./types').AdminRippleResponse>(`/api/admin/ripple?${new URLSearchParams({search,offset:String(offset),limit:"25"})}`,{headers:{Authorization:`Bearer ${token}`},signal}),
