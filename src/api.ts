...
export const api = {
  ...
  position: (wallet:string,id:string) => request<{balanceRaw:string;balanceUpdatedAt:number|null;valueUsd:number|null;quoteSymbol:string;note:string;pnl:{available:false;reason:string}|{available:true;quoteDecimals:number;costBasis:number;bought:number;sold:number;realized:number;unrealized:number|null;total:number|null;returnPercent:number|null}}>(`/api/wallets/${encodeURIComponent(wallet)}/positions/${encodeURIComponent(id)}`),
