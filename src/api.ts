...
export const api = {
  ...
  creatorFeeSummary: (id: string) => request<import("./types").CreatorFeeSummary>(`/api/launches/${encodeURIComponent(id)}/creator-fees`),
  ...
  confirmCreatorFees: (id:string,creator:string,signature:string)=>request<{confirmed:boolean;amountRaw:string}>(`/api/launches/${encodeURIComponent(id)}/creator-fees/confirm`,json({creator,signature})),
