...
export const api = {
  ...
  adminMarkProposalPaid: (token: string, id: string, orderReference?: string) => request<unknown>(`/api/admin/proposals/${encodeURIComponent(id)}/mark-paid`, { method: "POST", headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" }, body: JSON.stringify({ orderReference }) }),
  adminCompleteProposal: (token: string, id: string) => request<unknown>(`/api/admin/proposals/${encodeURIComponent(id)}/complete`, { method: "POST", headers: { Authorization: `Bearer ${token}` } }),
