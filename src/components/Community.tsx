...
function CommunityRoom({launch,onRead}:{launch:Launch;onRead?:(cursor:CommentCursor)=>void}){
  ...
  const [nearBottom,setNearBottom]=useState(true),[unseen,setUnseen]=useState(0),[mode,setMode]=useState<'message'|'update'>('message');
  const [menu,setMenu]=useState<CommunityPost|null>(null),[dialog,setDialog]=useState<'poll'|'report'|'delete'|null>(null),[target,setTarget]=useState<CommunityPost|null>(null),[lightbox,setLightbox]=useState<string|null>(null);
  ...
  useEffect(()=>{
    alive.current=true;first.current=true;retryAfter.current=0;postsRef.current=[];latest.current=null;atBottom.current=true;setPosts([]);setCursor(null);setUnseen(0);setNearBottom(true);void load();
  ...
  async function send(){
    ...
    const key=JSON.stringify([mode,body.trim(),reply?.id,file?.name,file?.lastModified,file?.size]);if(draft.current?.key!==key)draft.current={key,id:crypto.randomUUID()};
    ...
    try{
      ...
      if(mode==='update'){const {update}=await creatorApi.publish(launch.id,token,{id:payload.id,body:payload.body});post={...update,kind:'update',imageUrl:null,reply:null,reactions:[],poll:null};}
      else post=(await communityApi.post(launch.id,token,payload,file)).post;
      if(!alive.current)return;
      request.current?.abort();request.current=null;
      scrollMode.current='bottom';atBottom.current=true;setNearBottom(true);setPosts(p=>merge(p,[post]));setBody('');setReply(null);setFile(null);draft.current=null;setMode('message');
  ...
  async function createPoll(){
    ...
    try{const {post}=await communityApi.post(launch.id,token,{id:pollDraft.current.id,kind:'poll',body:question.trim(),options:options.map(s=>s.trim()),durationHours:hours},null);
      if(!alive.current)return;scrollMode.current='bottom';atBottom.current=true;setPosts(p=>merge(p,[post]));setDialog(null);setQuestion('');setOptions(['','']);pollDraft.current=null;setFilter('all');void load(null,true);
    }catch(e){if(alive.current)setActionError(e instanceof Error?e.message:'Poll could not be created.');}
    finally{busyRef.current=false;if(alive.current)setBusy(false);}
  }
  function startReply(post:CommunityPost){if(!requireSession())return;setReply(post);setMode('message');setMenu(null);text.current?.focus({preventScroll:true});}
  ...
  return <section ref={room} className="community" aria-label={`${launch.name} community`}>
        ...
        {posts.map((post,index)=>{const own=post.authorWallet===wallet.address,day=new Date(post.createdAt).toLocaleDateString(undefined,{month:'short',day:'numeric'}),newDay=!index||new Date(posts[index-1].createdAt).toDateString()!==new Date(post.createdAt).toDateString();const total=currentPollVotes(post),ended=Boolean(post.poll&&post.poll.closesAt<=Date.now());return <div key={post.id}>
          ...
          <article id={'community-'+post.id} className={`community-message ${own?'is-own':''} is-${post.kind}`}>
            ...
            <div className="community-bubble">
              ...
              <footer className="community-message-footer"><div className="community-reactions">{(post.reactions??[]).map(r=><button key={r.emoji} aria-label={`${r.emoji} reaction, ${r.count}`} aria-pressed={r.mine} disabled={busy} onClick={()=>void action(post,'react',{emoji:r.emoji,active:!r.mine})}>{r.emoji}<span>{r.count}</span></button>)}</div><button className="community-reply" aria-label="Reply to message" onClick={()=>startReply(post)}><Reply size={14}/></button><time dateTime={new Date(post.createdAt).toISOString()} title={new Date(post.createdAt).toLocaleString()}>{new Date(post.createdAt).toLocaleTimeString(undefined,{hour:'2-digit',minute:'2-digit'})}</time>{own&&<Check size={13} aria-label="Sent"/>}</footer>
              {menu?.id===post.id&&<div className="community-message-menu"><div className="community-quick-reactions">{['❤️','🔥','👍'].map(emoji=><button key={emoji} aria-label={`React ${emoji}`} onClick={()=>{void action(post,'react',{emoji,active:!post.reactions?.find(r=>r.emoji===emoji)?.mine});setMenu(null);}}>{emoji}</button>)}<button aria-label="Close message options" onClick={()=>setMenu(null)}><X size={15}/></button></div><button onClick={()=>startReply(post)}><Reply size={14}/>Reply</button>{moderator&&<button onClick={()=>{void action(post,'moderate',{action:'pin'});setMenu(null);}}><Pin size={14}/>Pin message</button>}<button onClick={()=>{setTarget(post);setDialog('report');setMenu(null);}}>Report</button>{(moderator||own)&&<button className="community-danger" onClick={()=>{setTarget(post);setDialog('delete');setMenu(null);}}>Delete message</button>}</div>}
            </div>
          </article>
        </div>;})}
    ...
    <div className="community-compose-area">
      ...
      {launch.status!=='live'?<p className="community-connect">Conversation opens when this coin launches.</p>:!wallet.address||!session?<div className="community-connect"><div><strong>Join the conversation</strong><span>Connect your wallet to chat, react and vote.</span></div><button onClick={()=>wallet.setModalOpen(true)}>Connect wallet<ArrowUpRight size={16}/></button></div>:<>
        {creator&&<div className="community-create-tools"><button aria-pressed={mode==='update'} disabled={sending} onClick={()=>{setMode(mode==='update'?'message':'update');setFile(null);setReply(null);}}><Megaphone size={14}/>{mode==='update'?'Writing an update':'Post update'}</button><button disabled={sending} onClick={()=>{setActionError('');setDialog('poll');}}><BarChart3 size={14}/>Create poll</button>{mode==='update'&&<button className="community-icon" aria-label="Cancel update" onClick={()=>setMode('message')}><X size={14}/></button>}</div>}
