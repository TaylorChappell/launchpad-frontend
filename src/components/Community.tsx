...
function pollTimeLeft(closesAt:number,now:number){
  const minutes=Math.max(1,Math.ceil((closesAt-now)/60000));
  if(minutes<60)return `Ends in ${minutes}m`;
  const hours=Math.floor(minutes/60),remaining=minutes%60;
  return `Ends in ${hours}h${remaining?` ${remaining}m`:''}`;
}
...
function CommunityRoom({launch,onRead}:{launch:Launch;onRead?:(cursor:CommentCursor)=>void}){
  ...
  const readOnly=Boolean(launch.showcase);
  const creator=!readOnly&&wallet.address===launch.creatorWallet,moderator=!readOnly&&Boolean(wallet.address&&wallet.address===config.adminWallet);
  ...
  const [postNow,setPostNow]=useState(Date.now);
  // Keep post ages and poll deadlines current even when background requests fail.
  useEffect(()=>{
    if(!posts.some(post=>post.kind!=='message'))return;
    const now=Date.now(),nextClose=Math.min(...posts.flatMap(post=>post.poll&&post.poll.closesAt>now?[post.poll.closesAt]:[]));
    const timer=window.setTimeout(()=>setPostNow(Date.now()),Math.max(1,Math.min(60000,nextClose-now)));
    const refresh=()=>{if(document.visibilityState==='visible')setPostNow(Date.now());};
    document.addEventListener('visibilitychange',refresh);
    return()=>{window.clearTimeout(timer);document.removeEventListener('visibilitychange',refresh);};
  },[posts,postNow]);
  const now=Math.max(postNow,Date.now());
  read.current=onRead;
  function markRead(){
  ...
  return <section ref={room} className="community" aria-label={`${launch.name} community`}>
    ...
    <div className={`community-conversation ${filter==='updates'||filter==='polls'?'community-post-feed':''}`}>
      <div className="community-scroll" ref={scroll} role="region" aria-label="Community messages" tabIndex={0} onScroll={()=>{const el=scroll.current!;const near=el.scrollHeight-el.scrollTop-el.clientHeight<70;atBottom.current=near;setNearBottom(near);if(near){setUnseen(0);markRead();}}}>
        ...
        {visiblePosts.map((post,index)=>{const isPost=post.kind!=='message',own=post.authorWallet===wallet.address,day=new Date(post.createdAt).toLocaleDateString(undefined,{month:'short',day:'numeric'}),newDay=!index||new Date(visiblePosts[index-1].createdAt).toDateString()!==new Date(post.createdAt).toDateString();const deliveryStatus=delivery.status(post.id),unconfirmed=deliveryStatus==='pending'||deliveryStatus==='failed';const total=currentPollVotes(post),ended=Boolean(post.poll&&post.poll.closesAt<=now);return <div key={post.id}>
          {!isPost&&newDay&&<div className="community-day"><span>{day}</span></div>}
          <article id={'community-'+post.id} className={`community-message ${isPost?'community-post':''} ${own?'is-own':''} is-${post.kind} ${deliveryStatus?'has-local-delivery':''}`} onContextMenu={event=>{event.preventDefault();if(unconfirmed||readOnly)return;const trigger=event.currentTarget.querySelector<HTMLElement>('.community-more')!;setMenu({post,x:event.clientX,y:event.clientY,trigger});}}>
            {isPost&&<span className="community-post-avatar" role="img" aria-label={`${launch.symbol} profile picture`}><TokenMark launch={launch}/></span>}
            {!own&&post.kind==='message'&&<CommentAvatar wallet={post.authorWallet}/>}
            <div className="community-bubble">
              <header>{isPost?<div className="community-post-byline"><b title={launch.name}>{launch.symbol}</b><span aria-hidden="true">·</span><time dateTime={new Date(post.createdAt).toISOString()} title={new Date(post.createdAt).toLocaleString()}>{buybackAge(post.createdAt,now)}</time></div>:!own&&<><WalletIdentity wallet={post.authorWallet} avatar={false} explorer={false}/>{post.authorWallet===launch.creatorWallet&&<span className="community-badge">Creator</span>}{post.authorWallet===config.adminWallet&&<span className="community-badge">AQUA team</span>}</>}<button disabled={unconfirmed||readOnly} className="community-icon community-more" aria-label="Message options" aria-haspopup="menu" aria-expanded={menu?.post.id===post.id} onClick={event=>{const trigger=event.currentTarget,rect=trigger.getBoundingClientRect();setMenu(menu?.post.id===post.id?null:{post,x:rect.right,y:rect.bottom+4,trigger});}}><MoreHorizontal size={17}/></button></header>
              ...
              {isPost&&post.body&&(post.kind==='update'?<UpdateText text={post.body} styled={post.bodyFormat==='styled'}/>:<MessageText text={post.body}/>)}
              ...
              {!isPost&&post.body&&<MessageText text={post.body}/>}
              {post.poll&&<div className="community-poll"><small>{post.poll.holdersOnly?'Holders only · minimum 0.1% of supply':'Community poll · one vote per wallet'}</small>{post.poll.options.map((option,i)=>{const percent=total?Math.round(option.votes/total*100):0;return <button key={i} disabled={busy||ended||readOnly} aria-pressed={post.poll!.myChoice===i} onClick={()=>void action(post,'vote',{choice:i})}><span className="community-poll-fill" style={{width:percent+'%'}}/><span>{post.poll!.myChoice===i?<Check size={15}/>:<span className="community-radio"/>}{option.label}</span><b>{percent}%</b></button>;})}<footer className={ended?'is-ended':''}><span>{total} {total===1?'vote':'votes'}</span><span className="community-poll-deadline"><span>{ended?<Check size={13}/>:<Clock3 size={13}/>}<b>{ended?'Poll ended':pollTimeLeft(post.poll.closesAt,now)}</b></span><time dateTime={new Date(post.poll.closesAt).toISOString()}>{ended?'Ended':'Ends'} {new Date(post.poll.closesAt).toLocaleString(undefined,{day:'numeric',month:'short',year:new Date(post.poll.closesAt).getFullYear()!==new Date(now).getFullYear()?'numeric':undefined,hour:'numeric',minute:'2-digit',timeZoneName:'short'})}</time></span></footer></div>}
    ...
    {!readOnly&&(filter==='all'||(creator&&launch.status==='live'&&(filter==='updates'||filter==='polls'))||notice||delivery.reactionError||(actionError&&!dialog))&&<div className="community-compose-area">
