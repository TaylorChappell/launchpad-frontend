...
export function TokenCard({ launch, sample = false, featured = false }: { launch: Launch; sample?: boolean; featured?: boolean }) {
  return <Link className={`token-card ${featured ? "featured" : ""}`} to={`/token/${launch.id}`}>
    ...
    <div className="market-status"><span className={launch.status}/><small>{launch.status === "orca" ? "Trading with Orca liquidity" : `${launch.progress.toFixed(0)}% toward Orca liquidity`}</small>{sample && <em>SAMPLE</em>}</div>
