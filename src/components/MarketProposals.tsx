...
export function MarketInformationTabs({section,onChange}:{section:string;onChange:(value:string)=>void}){
  ...
  const hasGovernance=Boolean(data?.enabled&&data.proposals.some(p=>p.isDefault?!["rejected","cancelled"].includes(p.status):(p.type==="cto"||p.type==="dex_boost")||!["rejected","cancelled"].includes(p.status)));
...
function BoostProposalCard({ proposal: p }: { proposal: MarketProposal }) {
  ...
  return <article className="proposal-vote-card boost-proposal-card">
    <header><div><DexScreenerIcon/><h3>DEX boost</h3></div><span className={"proposal-state " + p.status}>{p.status === "approved" ? "Queued" : p.status === "ready" ? "Ready to purchase" : p.status === "withdrawn" ? "Purchase pending" : p.status}</span></header>
    <h4>{title}</h4><p className="proposal-reason">{String(p.payload.reason ?? "")}</p>
    {voting ? <><div className="proposal-vote-meta"><span>{p.eligibleVoters} eligible voters</span><time>{countdown(p.endsAt, now)} left</time></div>
      ...
      {p.status === "funding" && <div className="proposal-funded"><div><span>{affordable ? `${affordable.boosts}x affordable` : "$99 minimum"}</span><b>${p.fundedUsd.toFixed(2)}</b></div><progress aria-label="Boost funding" value={p.fundedUsd} max={nextPack ? nextPack.cents / 100 : 3999}/><small>{nextPack ? `$${(nextPack.cents / 100).toLocaleString()} unlocks ${nextPack.boosts}x` : "Largest pack funded. Surplus returns to holders."}</small></div>}
      {p.status === "approved" && <p className="proposal-detail-note">The hour starts once DEX profile funding and any open challenges are resolved.</p>}
      {p.boostPack && <div className="boost-selected-pack"><b>{p.boostPack}x · {p.boostHours} hours</b><span>${p.targetUsd.toFixed(0)}</span></div>}
      {p.outcome === "below_minimum" && <p className="proposal-detail-note">The available reserve was below $99. All remaining funds returned to holder rewards.</p>}
    ...
    <details className="proposal-public-details"><summary>Boost packs & funding</summary><BoostPackPrices packs={packs}/><p>The largest affordable pack is selected when the hour ends. SOL is valued at settlement; affordability is checked again before withdrawal. Any remainder returns to holder rewards. AQUA completes the purchase and records its receipt.</p></details>
    {Boolean(p.openChallenges) && <p className="proposal-detail-note">Purchase paused while a holder challenge is reviewed.</p>}
    {["approved", "funding", "ready"].includes(p.status) && wallet.address && data.votePower?.eligible && <button className="proposal-text-action" disabled={busy} onClick={() => open({ kind: "challenge", proposal: p })}>Challenge proposal</button>}
  </article>;
}
...
export function CommunityProposalVotes() {
  ...
  const proposals = data.proposals.filter((item) => !item.isDefault && !(!["cto", "dex_boost"].includes(item.type) && ["rejected", "cancelled"].includes(item.status)));
  ...
  return createPortal(<div className="aqua-survey-backdrop" onMouseDown={(event) => { if (event.currentTarget === event.target) close(); }}><section className="aqua-survey" role="dialog" aria-modal="true" aria-labelledby="proposal-survey-title" ref={dialogRef} tabIndex={-1}>
    ...
    <form onSubmit={(event) => { event.preventDefault(); if (!step) review(); else if (accepted && !busy) void submit(content()); }}>
      {!step ? <div className="survey-fields">
        ...
        {kind === "create" && type === "dex_boost" && <div className="survey-funding-summary"><b>5% · 10% · 20% · No</b><p>Holders have 15 minutes to choose. The winning rate funds one hour, then AQUA purchases the largest affordable pack and returns the remainder to holders. Funding waits for any active DEX profile campaign.</p><BoostPackPrices/></div>}
