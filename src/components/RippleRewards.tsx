...
function RippleBalances({ address }: { address: string }) {
  ...
  return <>
    ...
    <div className="ripple-posts"><header><h3>Your posts</h3><span>Hourly rewards</span></header><p className="ripple-eligibility">Earn rewards from coins you hold in your linked wallet.</p>
      {data && (data.status === "paused" || data.status === "catching_up") && <p className="ripple-tracking-notice" role="status">Post updates are delayed. Your earned rewards are safe.</p>}
      ...
      {!data ? !error && <p className="ripple-empty">Loading your posts…</p> : !data.posts.length ? <p className="ripple-empty">No posts detected yet. Use the coin’s $ticker or contract address on X. We check hourly.</p> : data.posts.slice(0, visible).map(post => <article key={`${post.launchId}:${post.id}`}>
        ...
        <footer className="ripple-post-progress"><span>{post.reason ?? (post.trackingStatus === "completed" ? "Checks complete" : post.nextCheckAt ? `Next check ${checkTime(post.nextCheckAt)}` : post.status === "ready" ? "Awaiting hourly payout" : "Tracking engagement")}</span>
