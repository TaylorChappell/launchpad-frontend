...
function RippleBalances({ address }: { address: string }) {
  ...
  return <>
    ...
    <div className="ripple-posts"><header><h3>Your posts</h3><span>{data?.service?.settlementMinutes===15 ? "15-minute rewards" : "Hourly rewards"}</span></header><p className="ripple-eligibility">Earn rewards from coins you hold in your linked wallet.</p>
      ...
      {data && (!data.enabled || data.service?.message || data.status==="paused" || data.status==="catching_up") && <p className="ripple-tracking-notice" role="status">{!data.enabled ? data.reason : data.service?.message || data.scanError || (data.status==="paused" ? "Post updates are delayed. Your earned rewards are safe." : "Checking for your latest posts. Earned rewards remain available.")}</p>}
