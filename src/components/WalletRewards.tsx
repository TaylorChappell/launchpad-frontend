...
import { ArrowUpRight, Check, Gift, Loader2 } from "lucide-react";
import { api } from "../api";
...
function RewardContent({launch,data,launches,onClaimed}:{launch?:Launch;data?:WalletRewardsResponse|null;launches:Launch[];onClaimed?:()=>void}){
  ...
  const markets=(rewardData?.markets??[]).filter(m=>!launch||m.launchId===launch.id).sort((a,b)=>Number(b.canClaim)-Number(a.canClaim)||b.accumulatingUsdCents-a.accumulatingUsdCents);
