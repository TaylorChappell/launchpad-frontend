...
export function normalizeWalletSignIn(result: unknown): WalletSignInOutput {
  if (Array.isArray(result)) {
    if (result.length !== 1) throw new Error("The wallet did not return one sign-in account. Connect again.");
    result = result[0];
  }
  if (!result || typeof result !== "object") throw new Error("The wallet did not return a sign-in result. Connect again.");
  const output = result as Record<string, any>;
  let address = output.account?.address;
  ...
  try {
    ...
    throw new Error("The wallet did not return a valid Solana sign-in account. Connect again.");
  }
  ...
  if (!signedMessage?.length || signature?.length !== 64) throw new Error("The wallet returned an incomplete sign-in proof. Connect again.");
