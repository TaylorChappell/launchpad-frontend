...
export function Analytics(){
  ...
  useEffect(()=>{
    ...
    void load();const timer=window.setInterval(()=>{if(document.visibilityState==="visible"){setNow(Date.now());void load();}},15_000);
  ...
  return <main className="page holder-workspace analytics-workspace">
    <header className="workspace-heading"><div><h1>Analytics</h1></div></header>
    ...
    {!data?<div className="workspace-loading">{offline?"Analytics unavailable":"Loading AQUA activity…"}</div>:<>
      <section className="network-metrics">
        <article className="network-metric-featured"><span><Gift size={19}/> Holder rewards</span><strong>{usd.format(data.totals.rewardsAccumulatedUsd)}</strong><small>All-time total</small></article>
