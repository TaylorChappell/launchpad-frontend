...
export function Admin() {
  ...
  const proposalAction = async (id: string, action: "withdraw" | "paid" | "complete" | "uphold" | "reject") => {
    if (!token) return;
    ...
    try {
      if (action === "withdraw") {
        ...
        const reference = window.prompt("DEX order or payment reference (optional)") ?? undefined;
        await api.adminMarkProposalPaid(token, id, reference);
        toast.success("DEX payment marked complete");
      } else if (action === "complete") {
        await api.adminCompleteProposal(token, id);
  ...
  return <main className={`page admin-page ${config.marketGovernanceEnabled ? "" : "market-governance-disabled"}`}>
    ...
    <section className="admin-panel admin-proposals"><header><div><h2><Gavel/> Market proposals</h2><p>Review votes and challenges, withdraw a fully funded DEX reserve, then record the external action.</p></div><b>{data.proposals.filter((item) => ["voting", "funding", "approved", "ready", "withdrawn"].includes(item.status)).length} active</b></header><div className="admin-table-wrap"><table><thead><tr><th>Market / type</th><th>Status</th><th>Vote</th><th>DEX fund</th><th>Challenges</th><th>Admin action</th></tr></thead><tbody>{data.proposals.length ? data.proposals.map((proposal) => { const yes=BigInt(proposal.yesPowerRaw || "0"); const no=BigInt(proposal.noPowerRaw || "0"); const total=yes+no; const yesPct=total ? Number(yes*100n/total) : 0; return <tr key={proposal.id}><td><b>${proposal.marketSymbol}</b><small className="admin-proposal-type">{proposal.type !== "cto" && <DexScreenerIcon/>}{proposal.type.replaceAll("_", " ")}</small><SubmittedProposalDetails proposal={proposal}/><a href={explorer("account", proposal.mint)} target="_blank" rel="noreferrer">{short(proposal.mint)}<ExternalLink/></a></td><td><span className={`admin-status ${proposal.status}`}>{proposal.status}</span><small>{proposal.outcome ?? "Voting"}</small></td><td><b>{yesPct}% yes</b><small>{proposal.eligibleVoters} eligible voters</small></td><td>{proposal.type === "dex_payment" ? <><b>${proposal.fundedUsd.toFixed(2)} / ${proposal.targetUsd.toFixed(0)}</b><small>{sol(proposal.fundedLamports)}</small>{proposal.withdrawalSignature && <a href={explorer("tx", proposal.withdrawalSignature)} target="_blank" rel="noreferrer">Proof <ExternalLink/></a>}</> : "—"}</td><td>{proposal.openChallenges ?? 0}</td><td><div className="admin-proposal-actions">{proposal.status === "ready" && <button disabled={Boolean(proposalBusy)} onClick={() => void proposalAction(proposal.id, "withdraw")}>{proposalBusy === `${proposal.id}:withdraw` ? <Loader2 className="spin"/> : null}Withdraw</button>}{proposal.status === "withdrawn" && <button disabled={Boolean(proposalBusy)} onClick={() => void proposalAction(proposal.id, "paid")}>Mark paid</button>}{proposal.status === "approved" && proposal.type !== "dex_payment" && <button disabled={Boolean(proposalBusy)} onClick={() => void proposalAction(proposal.id, "complete")}>Mark complete</button>}{Boolean(proposal.openChallenges) && <><button className="danger" disabled={Boolean(proposalBusy)} onClick={() => void proposalAction(proposal.id, "uphold")}>Uphold</button><button disabled={Boolean(proposalBusy)} onClick={() => void proposalAction(proposal.id, "reject")}>Reject</button></>}</div></td></tr>; }) : <tr><td colSpan={6} className="admin-empty">No market proposals yet.</td></tr>}</tbody></table></div></section>
...
function SubmittedProposalDetails({ proposal }: { proposal: MarketProposal }) {
  ...
  const labels: Record<string, string> = { reason: "Reason", description: "Profile description", bannerUrl: "Banner", websiteUrl: "Website", xUrl: "X", telegramUrl: "Telegram", communityLead: "Community lead", multisig: "Multisig", plan: "Transition plan", evidenceUrl: "Public evidence" };
