  ...
  return <main className="page address-claim-page">
    ...
    {markets&&<section className="address-claim-results"><h2>Fees and rewards for {address.slice(0,5)}…{address.slice(-5)}</h2>
      ...
      {rewards?.markets.filter(m=>m.grossRedeemableUsdCents>0).map(m=>m.claimMode==="cumulative"
        ...
        : m.claimableEpochIds.map(epochId=>{const epoch=rewards.rewards.find(r=>r.epochId===epochId);return <article className="address-claim-market" key={epochId}><div><Link to={`/token/${m.launchId}`}>Holder rewards · {markets.find(x=>x.launchId===m.launchId)?.name??m.launchId.slice(0,8)}</Link><p>Reward epoch {epochId.slice(0,8)}</p><strong>{formatUsd(epoch?.amountUsdCents??0)}</strong>{!m.canClaim&&<small>Accumulating until the minimum claim amount is reached.</small>}</div><button className="primary" disabled={!rewardClaimsEnabled||!m.canClaim||busy} onClick={()=>void start(m.launchId,"legacy",epochId)}>Claim rewards <ArrowRight size={15}/></button></article>;}))}
