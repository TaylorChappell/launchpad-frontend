...
import { ArrowRight, Grid2X2, List, RefreshCw, Search, SlidersHorizontal, Star, X } from "lucide-react";
import { Link, useSearchParams } from "react-router-dom";
import { readMarketSnapshot, saveMarketSnapshot } from "../market-cache";
...
export function Markets(){
  ...
  const dex=params.get("dex")==="paid"?"paid":"all";
  const view=params.get("view")==="table"?"table":"cards", q=params.get("q")??"";
  const [filtersOpen,setFiltersOpen]=useState(false);
  const filterCount=Number(dex!=="all")+Number(pair!=="all")+Number(mode!=="all")+Number(Boolean(q));
  ...
  const query={sort:sort==="watchlist"?"volume":sort,pair,mode,dex,q,limit:24,...(sort==="watchlist"?{ids:watchKey}:{})};
  const queryKey=JSON.stringify(query);
  const cacheKey=API_URL+queryKey;
  const initial=useMemo(()=>readMarketSnapshot(cacheKey),[cacheKey]);
  ...
  useEffect(()=>{
    const controller=new AbortController();requestGeneration.current++;setLoadingMore(false);
    const cached=readMarketSnapshot(cacheKey);setDisplayedKey(queryKey);setState(cached?"ready":"loading");setLaunches(cached?.launches??[]);setMore(cached?.hasMore??false);setOffset(cached?.nextOffset??0);
    const timer=window.setTimeout(()=>{api.launches(JSON.parse(queryKey),controller.signal).then(data=>{if(!controller.signal.aborted){saveMarketSnapshot(cacheKey,data);setLaunches(data.launches);setMore(data.hasMore);setOffset(data.nextOffset);setState("ready");}}).catch(()=>{if(!controller.signal.aborted)setState("offline");});},q?200:0);
  ...
  const shown=(displayedKey===queryKey?launches:initial?.launches??[]).map(l=>mergeMarketPrice(l,prices.get(l.id)));
  ...
  return <main className="explore-page">
    ...
    <section className="market-workspace" id="markets">
      ...
      {filtersOpen&&<div className="discovery-filters" id="discovery-filters"><label className="discovery-search"><Search size={16}/><input aria-label="Search all markets" placeholder="Name, ticker or address" value={q} onChange={e=>update("q",e.target.value)}/></label><label htmlFor="discovery-pair">Pair<select id="discovery-pair" value={pair} onChange={e=>update("pair",e.target.value)}><option value="all">All pairs</option><option>SOL</option><option>ORCA</option><option value="STOCK">xStocks</option></select></label><label htmlFor="discovery-rewards">Rewards<select id="discovery-rewards" value={mode} onChange={e=>update("mode",e.target.value)}>{modes.map(([value,label])=><option key={value} value={value}>{label}</option>)}</select></label><label htmlFor="discovery-dex">DEX status<select id="discovery-dex" value={dex} onChange={e=>update("dex",e.target.value)}><option value="all">All coins</option><option value="paid">DEX paid</option></select></label>{filterCount>0&&<button className="discovery-reset" onClick={()=>{const next=new URLSearchParams(params);["pair","mode","dex","q"].forEach(key=>next.delete(key));setParams(next,{replace:true});}}>Clear filters <X size={13}/></button>}</div>}
