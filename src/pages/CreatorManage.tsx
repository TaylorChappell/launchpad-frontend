...
export function CreatorManage() {
  ...
  if (launch.status !== "live") return <main className="page manage-page"><Link className="back" to="/portfolio?tab=created"><ArrowLeft/>Your coins</Link><section className="manage-gate"><h1>Finish launching this coin first</h1><p>The creator dashboard opens once the launch is confirmed.</p><Link className="primary" to="/create">Resume launch</Link><button className="soft-button" onClick={refresh}>Check launch status</button></section></main>;
  return <main className="page manage-page creator-dashboard">
    ...
    <div className="creator-dashboard-layout">
      ...
      <div className="creator-dashboard-content" key={id + ":" + wallet.address}>
        ...
        {selected === "fees" && <><section className="creator-dashboard-intro"><h2>Creator fees</h2></section><CreatorFeeClaim launch={launch} onClaimed={refresh}/><CreatorFeeLock launch={launch} onChanged={refresh}/></>}
