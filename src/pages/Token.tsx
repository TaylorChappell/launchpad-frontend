...
export function Token() {
  ...
  return <MarketGovernanceProvider key={launch.id} launch={launch}><main className="page token-page">
    ...
    <div className="token-layout"><section className="token-main">
      ...
      <div className="segmented" aria-label="Market information">{["Transactions","Holders","Your position","Rewards","Project","Governance"].map(label=><button key={label} aria-pressed={section===label} onClick={()=>setSection(label)}>{label}</button>)}</div>
      {section==="Holders"&&<MarketHolders launch={launch}/>}
      ...
      {section==="Rewards"&&<><div className="info-grid single reward-mode-market-panel">
      ...
      <DexFundingVote/><Link to="/rewards">Your reward allocations and claim eligibility →</Link></>}
