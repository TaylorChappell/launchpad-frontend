...
The default API is `https://aquaapi.fun`. Set `VITE_API_URL` only when you need a different backend. Staging must keep its staging API URL. The former production Railway origin is upgraded to the canonical domain by the client; existing third-party integrations can continue calling the Railway origin.
