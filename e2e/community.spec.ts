...
test('messages retain failed drafts, support replies and image lightbox',async({page})=>{
 ...
 const field=room.getByLabel('Your message');state.failSend=true;await field.fill('This should stay until sent');await room.getByRole('button',{name:'Send message'}).click();await expect(room.getByRole('alert')).toContainText('Could not send');await expect(field).toHaveValue('');await expect(room.locator('.community-message').last()).toContainText('This should stay until sent');state.failSend=false;await room.getByRole('button',{name:'Retry send'}).click();await expect(room.locator('.community-message').last()).toContainText('This should stay');
...
test('messages appear before the server responds, keep the composer usable, and survive stale refreshes',async({page},info)=>{
 const state=await setup(page,'holder');const room=page.locator('.community'),field=room.getByLabel('Your message');
 let release!:()=>void;const gate=new Promise<void>(resolve=>release=resolve);const issued:any[]=[];
 await page.route('**/api/launches/coin/community',async route=>{if(route.request().method()!=='POST')return route.fallback();issued.push(route.request().postDataJSON());await gate;return route.fallback();});
 await field.fill('Hello');await room.getByRole('button',{name:'Send message'}).click();const bubble=room.locator('.community-message').filter({has:page.locator('.community-text',{hasText:/^Hello$/})});
 await expect(bubble).toBeVisible();await expect(bubble.getByLabel('Sending message',{exact:true})).toBeVisible();await expect(bubble.getByLabel('Sent',{exact:true})).toHaveCount(0);await expect(field).toHaveValue('');await expect(field).toBeEditable();
 await expect(bubble.locator('header .wallet-identity-address,header .x-identity')).toHaveCount(0);
 await field.fill('Another message without waiting');await room.getByRole('button',{name:'Send message'}).click();await expect(room.getByLabel('Sending message',{exact:true})).toHaveCount(2);await expect.poll(()=>issued.length).toBe(2);
 await room.getByRole('button',{name:'Refresh community'}).click();await expect(bubble).toBeVisible();await expect(room.getByLabel('Sending message',{exact:true})).toHaveCount(2);
 await room.getByRole('button',{name:'Updates',exact:true}).click();await expect(bubble).toHaveCount(0);await room.getByRole('button',{name:'Chat',exact:true}).click();await expect(bubble).toBeVisible();
 release();await expect(room.getByLabel('Sending message',{exact:true})).toHaveCount(0);await room.getByRole('button',{name:'Refresh community'}).click();await expect(bubble).toHaveCount(1);expect(state.posts.filter(p=>p.body==='Hello')).toHaveLength(1);
 await bubble.scrollIntoViewIfNeeded();await room.screenshot({path:`/tmp/community-smooth-${info.project.name}.png`});
});
test('failed sends retry the original ID and do not overwrite the next draft',async({page})=>{
 await setup(page,'holder');const room=page.locator('.community'),field=room.getByLabel('Your message');let release!:()=>void;const gate=new Promise<void>(resolve=>release=resolve);const attempts:any[]=[];
 await page.route('**/api/launches/coin/community',async route=>{if(route.request().method()!=='POST')return route.fallback();attempts.push(route.request().postDataJSON());if(attempts.length===1){await gate;return route.fulfill({status:503,json:{error:'Connection interrupted.'}});}return route.fallback();});
 await field.fill('Keep this message');await room.getByRole('button',{name:'Send message'}).click();await field.fill('My next draft');release();await expect(room.getByRole('button',{name:'Retry send'})).toBeVisible();await expect(field).toHaveValue('My next draft');await room.getByRole('button',{name:'Retry send'}).click();await expect(room.getByRole('button',{name:'Retry send'})).toHaveCount(0);await expect.poll(()=>attempts.length).toBe(2);expect(attempts[0].id).toBe(attempts[1].id);await expect(field).toHaveValue('My next draft');
});
test('reactions update instantly, coalesce rapid toggles and roll back rejected changes',async({page})=>{
 const state=await setup(page,'holder');const room=page.locator('.community'),post=room.locator('#community-04');let release!:()=>void;const gate=new Promise<void>(resolve=>release=resolve),calls:boolean[]=[];let fail=false;
 await page.route('**/api/launches/coin/community/04/react',async route=>{const active=route.request().postDataJSON().active;calls.push(active);if(fail)return route.fulfill({status:429,json:{error:'Please wait before reacting again.'}});if(calls.length===1)await gate;const p=state.posts.find(p=>p.id==='04')!;p.reactions=[{emoji:'👍',count:active?4:3,mine:active}];return route.fulfill({json:{post:p}});});
