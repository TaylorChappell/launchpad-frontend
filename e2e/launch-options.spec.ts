...
test('settings persist and send fee, Ripple, marketing and DEX choices',async({page},info)=>{
 ...
 await expect(page.getByLabel('Reward fee',{exact:true})).toHaveValue('100');
 await expect(page.getByLabel('Ripple share',{exact:true})).toHaveValue('1500');
 await page.getByLabel('Reward fee',{exact:true}).press('End');
 await page.getByLabel('Ripple share',{exact:true}).press('End');
 await expect(page.locator('.coin-fee-total dd')).toHaveText('6%');
...
test('default fees remain launchable before the program upgrade and Ripple can use its minimum',async({page})=>{
  ...
  await expect(page.getByPlaceholder('Aqua Robotics')).toHaveValue('Squid');
  for(let i=0;i<3;i++)await page.getByRole('button',{name:'Continue',exact:true}).click();
  await expect(page.getByLabel('Reward fee',{exact:true})).toBeDisabled();
  await expect(page.getByLabel('Reward fee',{exact:true})).toHaveValue('100');
  await page.getByLabel('Ripple share',{exact:true}).press('Home');
  await expect(page.getByLabel('Ripple share',{exact:true})).toHaveValue('300');
